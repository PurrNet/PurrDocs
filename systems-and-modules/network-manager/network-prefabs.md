...
The **Folder / Scene** field also accepts a scene. The registry then pulls the prefabs that scene references and regenerates when the scene is saved. With auto generate on, the list always mirrors what the scan finds, so prefabs that are no longer referenced are removed. This is the building block for [scene scoped prefabs](scene-scoped-prefabs-and-assets.md), which load and unload with their scene instead of living on the Network Manager.
